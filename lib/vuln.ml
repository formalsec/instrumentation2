...
(*   >>= fun conf0 -> *)
(*   match conf0.return with *)
(*   | None -> [ conf0 ] *)
(*   | Some conf1 -> unroll conf1 >>| fun conf -> { conf0 with return = Some conf } *)
...
module Parser : sig
  ...
  let from_json ?file (assoc : Json.t) : t Result.t =
    ...
    let* body = Util.member "body" assoc |> Vuln_ast.Parser.parse_body in
    Ok { ty; source; source_lineno; sink; sink_lineno; body }

  let from_file fname : t list Result.t =
